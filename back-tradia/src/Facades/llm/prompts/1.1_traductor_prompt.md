...
- Translate numbering and list markers into the target language (e.g., Chinese list markers like ï¼ˆä¸€ï¼‰, ï¼ˆäºŒï¼‰, ï¼ˆä¸‰ï¼‰ must be rendered in the target language as (1), (2), (3), or an equivalent numbering system). Do not leave source-language numerals or enumerators.
...
- Be conservative with vertical text: only rotate or use vertical writing-mode if the source clearly shows rotated/vertical glyphs. Do NOT rotate normal table cell text just because the column is narrow; keep table headers/row labels horizontal unless they are visibly rotated in the source.
...
- Vertical or rotated text (for example, labels beside tables or page-side headings) MUST:
  - Stay fully **inside the main content box** of the page (do not place it in the outer margin or padding area).
  - Be positioned so that its bounding box does **not overlap** other text or table cells; if necessary, slightly adjust its position or font size to avoid collisions.
  - Keep a small but visible gap (at least a few pixels) between the vertical text and any neighboring horizontal text or boxes, instead of drawing them on top of each other.
...
**Table Rules:**
- If the source page contains a table, you MUST render it as an HTML `<table>` with `<tr>` and `<td>` (or `<th>`) elements.
- Preserve the **exact table structure**: the same number of rows and the same number of columns in the same order. Do NOT transpose rows into columns (or vice versa).
- Do NOT drop, merge, or split rows/cells. If a cell is visually empty, keep it as an empty cell using `&nbsp;` so the grid stays intact.
- Use `rowspan` / `colspan` **only** when the source clearly shows merged cells. Otherwise, avoid spans.
- Preserve the table’s geometry: column widths, row heights, and overall position should visually match the source. If space is tight, adjust font size slightly but keep the grid.
- Keep text alignment in each cell consistent with the source (left/center/right; top/middle/bottom).
- If table borders or grid lines are visible in the source, render borders to match them. If borders are faint, a thin 1px border is acceptable, but the grid must remain clear.
