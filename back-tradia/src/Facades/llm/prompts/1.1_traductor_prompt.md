...
- For **vertical text that is true text content** (e.g., rotated labels or table headers, not part of a logo or barcode image):
  - You MUST translate the text normally.
  - You MUST preserve the vertical orientation using CSS on the container (for example, `writing-mode: vertical-rl;` or `transform: rotate(-90deg);`), so that the text remains vertical in the layout.
  - Only treat vertical text as an image region (with a dotted `div`) if it is clearly part of a logo, stamp, barcode strip, or other purely graphical mark.
